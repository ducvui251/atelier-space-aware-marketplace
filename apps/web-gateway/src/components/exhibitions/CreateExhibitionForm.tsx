...
import type { Exhibition, ExhibitionRoomTemplateId } from "@atelier/contracts";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const exhibition = await apiFetch<Exhibition>("/api/exhibitions", {
        method: "POST",
        body: JSON.stringify({ title, slug, description, roomTemplateId }),
      });
      router.replace(`/exhibitions/manage/${exhibition.id}`);
    } catch (cause) {
    ...
    <form onSubmit={handleSubmit} className="max-w-2xl rounded-lg border border-border bg-surface p-5 sm:p-7">
      <div className="grid gap-5">
        ...
        <fieldset className="grid gap-2">
          <legend className="text-body-sm font-medium text-foreground">Room style</legend>
          <p className="text-caption text-muted-foreground">Each style uses the same 10 m × 10 m room, so artwork positions fit consistently.</p>
        ...
        <div className="flex flex-wrap gap-3">
          <Button type="submit" disabled={busy || !title.trim() || !slug.trim()}>{busy ? "Creating…" : "Create draft"}</Button>
