...
function PendingArtistsQueue() {
  const { data, loading, error, refresh } = useApiResource<{ artists: Artist[]; artworks: unknown[] }>("/api/admin/verification-queue");
  ...
  return (
    <>
      ...
      <h1 className="mt-2 font-display text-h2 text-foreground">
        Artists pending review {loading ? "" : `(${pendingArtists.length})`}
      </h1>
      ...
      {error ? (
        ...
        <div className="mt-8">
          <EmptyState icon={UserCheck} title="No artists pending review" description="New artist signups will show up here." />
        </div>
      ) : (
        <div className="mt-8 flex flex-col gap-3">
