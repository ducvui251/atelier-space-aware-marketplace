...
import { reviewArtist } from "@/lib/gateway/clients/verification.client";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { return json(await reviewArtist(id, body)); }
  catch { return errorResponse("Artist verification failed", 409); }
}
