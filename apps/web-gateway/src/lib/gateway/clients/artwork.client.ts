...
import { artists as buildArtists, artworks as buildArtworks } from "@atelier/artist-artwork-service";
...
export async function listArtworks(): Promise<Artwork[]> {
  if (process.env.NEXT_PHASE === "phase-production-build") return buildArtworks;
...
export async function findArtwork(id: string): Promise<Artwork | null> {
  if (process.env.NEXT_PHASE === "phase-production-build") return buildArtworks.find((item) => item.id === id) ?? null;
...
export async function listArtists(): Promise<Artist[]> {
  if (process.env.NEXT_PHASE === "phase-production-build") return buildArtists;
...
export async function findArtist(id: string): Promise<Artist | null> {
  if (process.env.NEXT_PHASE === "phase-production-build") return buildArtists.find((item) => item.id === id) ?? null;
