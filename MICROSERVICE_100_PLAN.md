...
Status: **partial; G-22 closed with live durability/recovery proof. Broker connectivity for Commerce/Recommendation/Room Preview/Admin, full §7.3 event-catalog coverage, and pending-outbox metrics remain open** for a future pass — this pass focused on the single most correctness-critical, explicitly-named item (G-22) and did it thoroughly rather than spreading thin across the whole event catalog.

Tasks:

- [ ] Extend broker connectivity (`EVENT_BROKER_URL`, `EVENT_QUEUE_PREFIX`) to Commerce, Verification, Recommendation, Room Preview, Admin — update registries per §2 as each service adopts the variables. *(2026-09-09: Verification now has broker connectivity (its own outbox publisher) as part of the G-22 fix below. Commerce, Recommendation, Room Preview, and Admin still lack it — still open.)*
- [ ] Add a service-owned outbox usage where events are required (Commerce, Verification, Admin minimum). *(2026-09-09: Verification now has its own `verification.event_outbox` table and publisher, used for `ArtworkVerified.v1`/`ArtistVerified.v1` — see G-22 below. Commerce and Admin still have no outbox — still open.)*
- [ ] Implement publisher confirms, bounded retries, dead lettering, and pending-outbox metrics. *(Publisher confirms and dead-lettering already existed as shared infrastructure in `packages/events` before this pass and now also cover Verification's new outbox and Artist & Artwork's new consumer queue (`atelier.artist-artwork.v1` + its `.dlq`) for free, since they reuse the same `runOutboxPublisher`/`consumeEvents` code. Pending-outbox metrics still don't exist anywhere — still open.)*
- [ ] Implement durable consumers with event-ID deduplication for the full catalog in §7.3. *(2026-09-09: Artist & Artwork is now a consumer for the first time (previously producer-only), with its own `artist_artwork.processed_events` dedup table (migration 0013), consuming `ArtworkVerified.v1`/`ArtistVerified.v1` — see G-22. The rest of §7.3's catalog (`OrderCreated.v1`, `PaymentSucceeded.v1`, `PaymentFailed.v1`, `OrderShipped.v1`, `ShipmentUpdated.v1`, `ComplaintOpened.v1`) still has no producer or consumer implementation — still open.)*
...
- [ ] Populate the Catalog read model from events; keep the backfill worker for bootstrap. *(Unchanged this pass — Catalog was already populated from `artwork.published`/`artwork.reserved`/`artwork.sold` events plus its 60s poll backfill; it now additionally converges on verification changes too, as evidenced above, but that was a side effect of the G-22 work, not a new populate-from-events mechanism.)*
- [ ] Add duplicate, out-of-order, broker-down, and replay tests. *(Broker-down + recovery proved live for the new G-22 path, above. Duplicate-delivery and out-of-order tests for the new consumer specifically were not run — see the dedup note above for why that was judged low-value (shared, already-proven mechanism) rather than skipped for time. Still open for the rest of the event catalog.)*
- [ ] Record each event producer/consumer in the contract documentation and §2.5. *(Not done this pass — the §7.3 table in this document was used as the source of truth but wasn't itself updated to mark `ArtworkVerified.v1`/`ArtistVerified.v1` as consumed by Artist & Artwork now, in addition to Catalog. Still open.)*
