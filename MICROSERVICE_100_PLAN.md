...
| `SUPABASE_STORAGE_BUCKET` | removed (G-05) | Gateway upload relay | Fixed external bucket `artwork-images`; provision the public bucket and owner-scoped Storage INSERT policy per `runbooks/image-upload-setup.md`. |
...
- [x] Add the upload contract: `POST /api/uploads/image` public spec, Gateway relay, MIME/size rules, fixed bucket, and owner-scoped Storage policy (G-05, §9.3). *(2026-09-12: migrated the upload relay from the service-role key to the cookie-bound Supabase session and `storage.objects` INSERT RLS scoped to `artist-<auth.uid()>`; removed `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_STORAGE_BUCKET`; uploads now use a 10-second timeout and SHA-256 paths for safe retries. The form accepts files only and keeps the returned URL as a hidden value with an image preview. One-time bucket/policy setup remains in `runbooks/image-upload-setup.md`.)*

**Historical update 2026-09-09:** the user provisioned the public bucket and service-role configuration, and the prior service-role implementation was tested through the real bucket and artwork form. **Superseded 2026-09-12:** the Gateway no longer uses that key; it now writes with the artist's cookie-bound Supabase session and Storage RLS. The earlier test does not verify the current owner-folder policy, so apply the runbook policy and repeat the upload proof before claiming deployed completion.
...
Status: **partial foundation; not accepted**. Gateway pages and room mutations exist; upload code now uses cookie-bound Storage RLS but still needs a fresh live policy verification. Signup correctness, dependency/error states, rejected labels and browser E2E remain open.
...
- [ ] **Artist image upload is verified under the current RLS flow.** The Gateway upload code and file-only form are implemented, but the earlier 2026-09-09 browser proof exercised the retired service-role path; applying the runbook policy and a fresh session-bound upload proof remain open.
