...
Status: **partial; route-registry parity, boundary-schema coverage, error-code/retryable standardization, and per-mutation semantics documentation closed. Upload contract (G-05) and the G-19 identity fix remain open** — those are the two largest remaining items and are deliberately left for a dedicated pass each (G-19 in particular touches the shared internal-auth trust model used by all 8 services and warrants its own careful review, not a rushed fix bundled into this one).
...
- [ ] Add the upload contract: `POST /api/uploads/image` public spec + internal relay behavior, MIME/size rules, bucket name — register the bucket variable (G-05, §9.3). *(Not started this pass — deliberately deferred; no image-upload feature exists yet to contract against.)*
...
- [ ] Fix the identity contract (G-19): `/v1/account/me` GET/PATCH accept a validated principal, not a query-param user ID. *(Confirmed still open and re-scoped this pass: `authUserId` is a plain, caller-supplied query param on both `GET`/`PATCH /v1/account/me` (`account-service/src/server.ts`) — the Gateway currently derives it from a validated Supabase session before forwarding it, but any other caller inside the shared internal-token trust domain (i.e. any of the 8 services, or a bug in one of them) can pass an arbitrary `authUserId` and read/update a different user's profile; the same caller-supplied-ID pattern also appears as `buyerId`/`artistId` query params on several Commerce/Recommendation/Room Preview routes. A real fix needs a signed, Gateway-asserted identity header (or equivalent) that a service can independently verify rather than trust at face value — an architectural addition to the shared-secret internal-auth model used by all 8 services, not a one-file patch, so it's kept as its own dedicated pass rather than rushed here.)*
