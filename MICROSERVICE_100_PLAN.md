...
- Producers (2026-09-09): artist-artwork, verification (G-22), commerce (`OrderCreated.v1`/`PaymentSucceeded.v1`).
- Consumers (2026-09-09): catalog-discovery (unchanged), artist-artwork (G-22, consumes Verification's events), admin (new, consumes Commerce's events into `admin.order_feed`).
...
Status: **partial; G-22 closed with live durability/recovery proof. Commerce and Admin now also have real broker connectivity, an outbox, and a live producer→consumer pair (`OrderCreated.v1`/`PaymentSucceeded.v1` → `admin.order_feed`), proved through a real Stripe test-mode checkout. Recommendation/Room Preview broker connectivity, the rest of §7.3's event catalog, and pending-outbox metrics remain open** for a future pass.
...
- [ ] Implement durable consumers with event-ID deduplication for the full catalog in §7.3. *(2026-09-09: Admin is now a consumer for the first time, with its own `admin.processed_events` dedup table (migration `0014`), consuming `OrderCreated.v1`/`PaymentSucceeded.v1` into a new read model, `admin.order_feed` (upserted so either event alone can create a correct row, self-healing regardless of delivery order). `GET /v1/admin/stats` was deliberately left untouched — it still computes `totalOrders`/`revenue` via the existing synchronous call to `commerce-service`, so this pass adds a new capability without risking a working, financially-relevant endpoint; no new Gateway/UI route exposes `order_feed` yet, so it's proven by direct DB verification, not through a public API — that wiring is left for Phase 7. The rest of §7.3's catalog (`PaymentFailed.v1`, `OrderShipped.v1`, `ShipmentUpdated.v1`, `ComplaintOpened.v1`) still has no producer or consumer implementation — still open.)*
