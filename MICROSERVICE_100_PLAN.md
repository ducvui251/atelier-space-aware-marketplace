...
- [x] Wire the upload UI: `ArtworkForm` multipart → `POST /api/uploads/image` → returned URL into create/update (G-05). *(Done ahead of schedule in Phase 4, 2026-09-09 — see that phase's evidence note. Still gated on the user completing the one-time Supabase Storage dashboard setup in `runbooks/image-upload-setup.md` before a real upload can succeed end-to-end; the URL-paste fallback works regardless.)*
...
- [ ] **Artist image upload works**; URL paste is secondary. *(Code + UI done, Phase 4, 2026-09-09; validation layers — auth, role, MIME, size — proven live end-to-end. Not checked here because the actual write to Supabase Storage is still blocked on the user completing `runbooks/image-upload-setup.md`'s one-time dashboard step; that step is outside this session's access.)*
