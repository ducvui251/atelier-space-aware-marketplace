...
const routes: Record<string, ServiceRouteHandler> = {
  "POST /v1/verification/artworks/:id/review": async ({ request, url, response, correlationId }) => {
    ...
    return result ? writeServiceJson(response, 200, result, correlationId) : writeServiceError(response, 404, { code: "NOT_FOUND", message: "Artwork not found", correlationId });
  },
  "POST /v1/verification/artists/:id/review": async ({ request, url, response, correlationId }) => {
    ...
    return result ? writeServiceJson(response, 200, result, correlationId) : writeServiceError(response, 404, { code: "NOT_FOUND", message: "Artist not found", correlationId });
  },
};
